...
namespace Cpdhelpdesk.Pages
{
    public partial class IHelpDeskTicketsComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Grid0LoadData(LoadDataArgs args)
        {
            try
            {
                var authenticationconnGetHelpDeskTicketsResult = await Authenticationconn.GetHelpDeskTickets(filter:$@"(contains(TicketGUID,""{search}"") or contains(TicketDescription,""{search}"") or contains(TicketRequesterEmail,""{search}"") or contains(assignTo,""{search}"") or contains(AuditCC,""{search}"")) and {(string.IsNullOrEmpty(args.Filter)? "true" : args.Filter)} and TicketRequesterUser eq ""{Security.User.Name}"" and contains(TicketStatus,""New"")", orderby:$@"{args.OrderBy}", expand:$"LocationList,ServiceCatglist,ServicesList", top:args.Top, skip:args.Skip, count:args.Top != null && args.Skip != null);
