...
namespace Cpdhelpdesk.Pages
{
    public partial class HelpDeskTicketsComponent : ComponentBase
    {
        ...
        {
            var dialogResult = await DialogService.OpenAsync<AddHelpDeskTicket>("Add Help Desk Ticket", null);
            await grid0.Reload();
        ...
        protected async System.Threading.Tasks.Task Splitbutton0Click(RadzenSplitButtonItem args)
        {
            if (args?.Value == "csv")
            {
                await Authenticationconn.ExportHelpDeskTicketsToCSV(new Query() { Filter = $@"{(string.IsNullOrEmpty(grid0.Query.Filter)? "true" : grid0.Query.Filter)}", OrderBy = $"{grid0.Query.OrderBy}", Expand = "HelpDeskStatus,LocationList,ServiceCatglist,ServicesList", Select = "Id,TicketGUID,HelpDeskStatus.TicketStatusDesc as HelpDeskStatusTicketStatusDesc,TicketDate,TicketDescription,TicketRequesterUser,TicketRequesterEmail,LocationList.locationDesc as LocationListlocationDesc,ServiceCatglist.ServiceCatgDesc as ServiceCatglistServiceCatgDesc,ServicesList.ServiceDesc as ServicesListServiceDesc,assignTo,AuditCC,TicketResponseUser" }, $"Help Desk Tickets");

            }

            if (args == null || args.Value == "xlsx")
            {
                await Authenticationconn.ExportHelpDeskTicketsToExcel(new Query() { Filter = $@"{(string.IsNullOrEmpty(grid0.Query.Filter)? "true" : grid0.Query.Filter)}", OrderBy = $"{grid0.Query.OrderBy}", Expand = "HelpDeskStatus,LocationList,ServiceCatglist,ServicesList", Select = "Id,TicketGUID,HelpDeskStatus.TicketStatusDesc as HelpDeskStatusTicketStatusDesc,TicketDate,TicketDescription,TicketRequesterUser,TicketRequesterEmail,LocationList.locationDesc as LocationListlocationDesc,ServiceCatglist.ServiceCatgDesc as ServiceCatglistServiceCatgDesc,ServicesList.ServiceDesc as ServicesListServiceDesc,assignTo,AuditCC,TicketResponseUser" }, $"Help Desk Tickets");
        ...
        protected async System.Threading.Tasks.Task Grid0RowDoubleClick(DataGridRowMouseEventArgs<Cpdhelpdesk.Models.Authenticationconn.HelpDeskTicket> args)
        {
            var dialogResult = await DialogService.OpenAsync<EditHelpDeskTicket>("Edit Help Desk Ticket", new Dictionary<string, object>() { {"Id", args.Data.Id} });
            await grid0.Reload();
        ...
        protected async System.Threading.Tasks.Task GridDeleteButtonClick(MouseEventArgs args, dynamic data)
        {
            try
            {
                if (await DialogService.Confirm("Are you sure you want to delete this record?") == true)
                {
                    ...
                    if (authenticationconnDeleteHelpDeskTicketResult != null && authenticationconnDeleteHelpDeskTicketResult.StatusCode == System.Net.HttpStatusCode.NoContent)
                    {
                        await grid0.Reload();
                    }
