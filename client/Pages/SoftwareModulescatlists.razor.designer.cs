...
namespace Cpdhelpdesk.Pages
{
    public partial class SoftwareModulescatlistsComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Splitbutton0Click(RadzenSplitButtonItem args)
        {
            if (args?.Value == "csv")
            {
                await Authenticationconn.ExportSoftwareModulescatlistsToCSV(new Query() { Filter = $@"{(string.IsNullOrEmpty(grid0.Query.Filter)? "true" : grid0.Query.Filter)}", OrderBy = $"{grid0.Query.OrderBy}", Expand = "", Select = "sprModulecatid,sprModulecatDesc,sprModulecatDesc1,FuturecatDesc,FuturecatDesc1,photopath,InstallcatModule" }, $"Software Modulescatlists");

            }

            if (args == null || args.Value == "xlsx")
            {
                await Authenticationconn.ExportSoftwareModulescatlistsToExcel(new Query() { Filter = $@"{(string.IsNullOrEmpty(grid0.Query.Filter)? "true" : grid0.Query.Filter)}", OrderBy = $"{grid0.Query.OrderBy}", Expand = "", Select = "sprModulecatid,sprModulecatDesc,sprModulecatDesc1,FuturecatDesc,FuturecatDesc1,photopath,InstallcatModule" }, $"Software Modulescatlists");
        ...
        protected async System.Threading.Tasks.Task Grid0RowDoubleClick(DataGridRowMouseEventArgs<Cpdhelpdesk.Models.Authenticationconn.SoftwareModulescatlist> args)
        {
            var dialogResult = await DialogService.OpenAsync<EditSoftwareModulescatlist>("Edit Software Modulescatlist", new Dictionary<string, object>() { {"sprModulecatid", args.Data.sprModulecatid} });
            await grid0.Reload();
        ...
        protected async System.Threading.Tasks.Task GridDeleteButtonClick(MouseEventArgs args, dynamic data)
        {
            try
            {
                if (await DialogService.Confirm("Are you sure you want to delete this record?") == true)
                {
                    ...
                    if (authenticationconnDeleteSoftwareModulescatlistResult != null && authenticationconnDeleteSoftwareModulescatlistResult.StatusCode == System.Net.HttpStatusCode.NoContent)
                    {
                        await grid0.Reload();
                    }
